...
const HttpStatus = {
    ...
    INTERNAL_SERVER_ERRROR: { code: 500, status: 'OINTERNAL_SERVER_ERRROR' }
}

export const getPatients = (req, res) => {
    logger.info(`${req.method} ${req.originalUrl}, fetching patients`)
    database.query(QUERY.SELECT_PATIENTS, (err, result) => {
        if (!result)
...
export const createPatient = (req, res) => {
    ...
    database.query(QUERY.CREATE_PATIENT, Object.values(req.body), (err, result) => {
        if (!result) {
            logger.error(error.message)
            res.status(HttpStatus.INTERNAL_SERVER_ERRROR.code)
                .send(new Response(HttpStatus.INTERNAL_SERVER_ERRROR.code, HttpStatus.INTERNAL_SERVER_ERRROR.status, 'Error occurred'))
        }
        else {
...
export const getPatient = (req, res) => {
    ...
    database.query(QUERY.SELECT_PATIENT, [ req.params.id ], (err, result) => {
        if (!result[0]) {
...
export const updatePatient = (req, res) => {
    ...
    database.query(QUERY.SELECT_PATIENT, [ req.params.id ], (err, result) => {
        if (!result[0]) {
            ...
            database.query(QUERY.UPDATE_PATIENT, [ ...Object.values(req.body), req.params.id ], (err, result) => {
                if (!err) {
                    res.status(HttpStatus.OK.code)
...
export const deletePatient = (req, res) => {
    ...
    database.query(QUERY.DELETE_PATIENT, [ req.params.id ], (err, result) => {
